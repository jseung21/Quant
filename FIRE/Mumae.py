...
###################### 값 설정 #########################
# 해당 주식
stock_list = ['BULZ','LABU','WEBL']
# 초기 금액
init_cash = 100000
# 백테스트 시작 날짜
back_test_start_date = '2021-09-01'
# 백테스트 기간 (1년 평균 : 240)
test_priod = 480 
# 평단가 이하 경우 매수 전략 (1: 하루치 전향 매수) 해당 수치 곱하기 개념
under_price_buy_strategy = 2
# 평단가 이상 경우 매수 전략 (0.5: 하루치 절반 매수) 해당 수치 곱하기 개념
upper_price_buy_strategy = 0
# 목표 수익률 매도 (1.1: 10% 수익일 경우 매도)
aim_plus_ratio = 1.1
# 40회차후 매도 전략 (1.05 : 5% 수익시 매도)
after_40th_buy_strategy = 1.05
########################################################


### 백테스트 시작
def start_back_test (stock, data):
    ...
    #         print("\n 1 -----------------------------------------------")
        ...
        # 40회차 이내 경우
            ...
            # 목표수익률 일때 전량 매도
            if _avg_px * aim_plus_ratio <= _cur_px :
    #             print("\n 2 -----------------------------------------------")
                _before_cash = cur_score.cash
                _after_cash = _before_cash + (_cur_px * _hold_nm)
                _nth = 0
    ...
    # TODO : yahoo 에서 가지고 오는 날짜 값 확인 필요
    ...
    p1, = ax.plot(score.est_val[1:], "b-")
    p2, = twin1.plot(score.cur_px[1:], "r-")
    p3, = twin2.plot(score.nth[1:], "g--", linestyle='--')
...
for stock in stock_list :
    ...
    # 백테스트 시작
    start_back_test(stock, data)

    #
    img = openpyxl.drawing.image.Image('myplot.png')
    img.anchor = 'A'+str(cell_row)
    cell_row += 35
    ws.add_image(img)
